...
class Problems {
  ...
  */
  ...
  // Average Time Complexity: O(Nloglog(N))
  // Worst Case Time Complexity: O(N^2)
  // Auxiliary Space Complexity: O(N)
  public static ArrayList<Double> bucketsort(ArrayList<Double> arr, Double min, Double max) {

    // If the input ArrayList is smaller than 50, just return it sorted
    if (arr.size() < 50) {
    ...
    int numberOfBuckets = (int)Math.ceil(Math.sqrt(arr.size()));
    Double bucketRange = (max - min) / numberOfBuckets;
    ArrayList<Double> result = new ArrayList<Double>();
    int bucketIndex;


    // Buckets: ArrayList of ArrayLists of Doubles
    ArrayList<ArrayList<Double>> buckets = new ArrayList<ArrayList<Double>>();
    // Individual entry in bucket: ArrayList of Doubles
    ArrayList<Double> b;
    ...
    for (int i = 0; i < arr.size(); i++) {
      bucketIndex = (int)Math.floor((arr.get(i) - min) / bucketRange);
      buckets.get(bucketIndex).add(arr.get(i));
    }


    // Loop through ArrayList of buckets and concatenate results of each
    for (int i = 0; i < buckets.size(); i++) {
        result.addAll(bucketsort(buckets.get(i), i * bucketRange, i * 2 * bucketRange));
    }
  ...
  */
  ...
  public static int kthSmallest(int[] arr, int k) {
    ...
    int[] intCounts = new int[8001];
    for(int i = 0; i < intCounts.length; i++){
      intCounts[i] = 0;
    }

    for (int i = 0; i < arr.length; i++){
      intCounts[arr[i] - 1000]++;
    }


    int[] cumulative = new int[8001];
    int prev = 0;

    for (int j = 0; j < intCounts.length; j++) {
      cumulative[j] = intCounts[j] + prev;
      prev += intCounts[j];
    }
